...
class HelloEntity extends PersistentEntity {
  ...
  override def behavior: Behavior = {
    ...
    }.onReadOnlyCommand[HelloSecond, String] {

      // Command handler for the Hello command
      case (HelloSecond(name, second), ctx, state) =>
        // Reply with a message built from the current message, and the name of
        // the person we're meant to say hello to.
        ctx.reply(s"$message, $name, $second!")
...
case class HelloSecond(name: String, second: String) extends HelloCommand[String]
